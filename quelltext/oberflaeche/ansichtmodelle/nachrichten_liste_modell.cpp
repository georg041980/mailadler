...
QVariant NachrichtenListeModell::data(const QModelIndex &index, int rolle) const {
    if (!index.isValid() || index.row() >= m_nachrichten.size()) return {};
    const auto &n = m_nachrichten.at(index.row());
    switch (rolle) {
    case BetreffRolle:  return n.betreff;
    case AbsenderRolle: return n.absender;
    case DatumRolle:    return n.datum;
    case GelesenRolle:  return n.gelesen;
    default: return {};
    }
}

QHash<int, QByteArray> NachrichtenListeModell::roleNames() const {
    return {{BetreffRolle, "betreff"}, {AbsenderRolle, "absender"},
            {DatumRolle, "datum"}, {GelesenRolle, "gelesen"}};
}

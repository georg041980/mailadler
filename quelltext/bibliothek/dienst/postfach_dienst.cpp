...
PostfachDienst::PostfachDienst(Speicher::Zwischenspeicher* cache, QObject* eltern) : QObject(eltern), m_cache(cache)
{
}
...
void PostfachDienst::setzeImapVerbindung(Protokoll::ImapVerbindung* imap)
{
    ...
    if (m_imap)
    {
        ...
        connect(m_imap, &Protokoll::ImapVerbindung::nachrichtHeaderEmpfangen, this,
...
void PostfachDienst::beiOrdnerAusgewaehlt(int zaehler)
{
    if (zaehler > 0)
    {
        ...
        if (m_db)
            m_db->nachrichtenLoeschenFuerOrdner(m_aktuellerOrdner);
...
{
    if (m_db)
        m_db->nachrichtSpeichern(nachricht);

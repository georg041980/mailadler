...
SmtpVerbindung::~SmtpVerbindung() { if (m_verbindung) m_verbindung->disconnectFromHost(); }

void SmtpVerbindung::setzeServer(const QString &s) { m_server = s; }
void SmtpVerbindung::setzePort(quint16 p)           { m_port = p;   }
void SmtpVerbindung::setzeTls(bool aktiv)           { m_tls = aktiv; }
QString SmtpVerbindung::server() const { return m_server; }
quint16 SmtpVerbindung::port()   const { return m_port;   }
...
void SmtpVerbindung::verbinden()
{
    ...
    if (!m_verbindung) {
        ...
        if (m_tls) {
            ...
            connect(m_verbindung, &QSslSocket::connected, this, [this]() {
        ...
        connect(m_verbindung, QOverload<QAbstractSocket::SocketError>::of(&QSslSocket::errorOccurred),
                this, &SmtpVerbindung::beiSocketFehler);
...
void SmtpVerbindung::verarbeiteAntwortZeile(const QByteArray &zeile)
{
    ...
    if (zeile.length() >= 4 && zeile[3] == '-') return; // Fortsetzungszeile
    ...
    case Phase::Quit:
        ...
        if (m_verbindung) m_verbindung->disconnectFromHost();
        break;
...
{
    if (zeile.length() < 3) return false;
    char c = zeile[0];
    return c == '2' || c == '3';
}
